...
//        }
    ...
    private func sortPortfolioCoinsIfNeeded(coins: [CoinModel]) -> [CoinModel] {
        // will only sort by holdings or oldingsReverse if needed
        switch sortOption {
        case .holdings:
    ...
    private func mapGlobalMarket(marketDataModel: MarketDataModel?, portfolioCoins: [CoinModel]) -> [StatisticModel] {
        ...
        let portfolioValue = portfolioCoins.map({ return $0.currentHoldingsValue })
            .reduce(0, +)
        ...
        let portfolio = StatisticModel(title: "Portfolio value", value: portfolioValue.asCurrencyWith2Decimals(), percantageChange: percentageChange)
